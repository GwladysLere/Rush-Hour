...
def inspect(names=False, show=True, wrap=True, field=80):
  ...
  # trunc: cut the middle part of 'val' to get a string of length 'rsize'
  trunc = lambda val: val[:rsize//2-2] + ' ... ' + val[-rsize//2+3:]
  # split: split and wrap 'val' to get a set of strings of length 'rsize' 
  split = lambda val: pformat(val,width=rsize,compact=True).replace('\n',offset)
  rhand = [repr(val) if len(repr(val)) <= rsize else \
           split(val) if wrap else trunc(repr(val)) for val in values]
